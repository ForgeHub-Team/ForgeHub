...
export function BranchCheckInsPage() {
  const { data, loading, error, reload } = useApi(checkInsApi.getCheckInHistory, []);
  ...
  return (
    <>
      ...
      <div className="mb-4 grid gap-4 md:grid-cols-3">
        <KpiCard label="Recent check-ins" value={rows.length} />
        ...
        columns={[
          ...
          { key: "source", label: "Source" },
          {
            ...
            render: (row) => row.isSuspicious
              ? <Badge tone={alertTone(row.suspicionLevel)}>{row.suspicionReason || "Possible fraud"}</Badge>
