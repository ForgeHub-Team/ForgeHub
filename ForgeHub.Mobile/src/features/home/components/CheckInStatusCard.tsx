  ...
  return (
    <ForgeCard style={styles.card}>
      <View style={styles.header}>
        <View>
          <Text style={styles.kicker}>Gym session</Text>
          <Text style={styles.title}>{hasActive ? "You are checked in" : "No active session"}</Text>
        </View>
        <Text style={[styles.badge, hasActive ? styles.badgeActive : styles.badgeIdle]}>{hasActive ? "Inside" : "Ready"}</Text>
      </View>
      {hasActive ? (
        <>
          ...
          <Text style={styles.text}>{session?.branchName ?? "Branch"} - {session?.status ?? "Inside Gym"}</Text>
          {checkoutMutation.error ? <Text style={styles.error}>{parseApiError(checkoutMutation.error).message}</Text> : null}
        ...
        <>
          <Text style={styles.text}>Scan your branch QR code when you arrive. ForgeHub will reload your active database session whenever the app opens.</Text>
...
function TimerFact({ label, value, muted = false }: { label: string; value: string; muted?: boolean }) {
  return (
    <View style={styles.fact}>
      ...
      <Text style={[styles.factLabel, muted ? styles.factMuted : null]}>{label}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  kicker: { color: colors.primary, fontSize: 12, fontWeight: "900", textTransform: "uppercase", letterSpacing: 0 },
  title: { color: colors.text, fontSize: 18, fontWeight: "900", letterSpacing: 0 },
  text: { color: colors.warm, lineHeight: 20, fontWeight: "700" },
  badge: { paddingHorizontal: 10, paddingVertical: 7, borderRadius: 8, overflow: "hidden", fontSize: 12, fontWeight: "900" },
  badgeActive: { backgroundColor: colors.primary, color: colors.white },
  badgeIdle: { backgroundColor: colors.surface2, color: colors.warm },
  timerBlock: { borderRadius: 8, backgroundColor: colors.primary, padding: 18 },
  timer: { color: colors.white, fontSize: 36, fontWeight: "900", letterSpacing: 0 },
  timerLabel: { color: colors.white, fontSize: 12, fontWeight: "900", marginTop: 2, textTransform: "uppercase", letterSpacing: 0 },
  grid: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  fact: { width: "48%", borderRadius: 8, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface2, padding: 12, gap: 4 },
  factValue: { color: colors.text, fontSize: 15, fontWeight: "900", letterSpacing: 0 },
  factLabel: { color: colors.warm, fontSize: 11, fontWeight: "800" },
  factMuted: { color: colors.secondary },
